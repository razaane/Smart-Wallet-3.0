...
require "classes/incomes.php";       // class

$user_id = $_SESSION['user_id']; // تأكدي راه متخزن فـ login
$db = new Database();
$pdo = $db->getConnection();

$income = new Income($pdo);
$result_incomes = $income->getAll($user_id);
?>
  ...
  <div class="flex min-h-screen">
    ...
    <aside class="hidden lg:flex lg:flex-col lg:w-[20%] bg-[color:var(--card-bg)] border-r border-gray-200 dark:border-gray-800 fixed h-full">
      ...
      <nav class="flex-1 p-4 space-y-2">
        ...
        <a href="affichage_inc.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-[color:var(--accent-light)] text-white transition-all hover:opacity-90">
          <span class="material-symbols-outlined">trending_up</span>
          <span class="font-medium">View Incomes</span>
        </a>
        
        <a href="affichage_exp.php" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 transition-all">
    ...
    <div id="mobile-sidebar" class="hidden fixed inset-0 z-50 lg:hidden">
      ...
      <aside class="absolute left-0 top-0 h-full w-64 bg-[color:var(--card-bg)] border-r border-gray-200 dark:border-gray-800 flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-2">
          ...
          <a href="affichage_inc.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-[color:var(--accent-light)] text-white transition-all">
            <span class="material-symbols-outlined">trending_up</span>
            <span class="font-medium">View Incomes</span>
          </a>
