...
$result_expenses = $expense->getAll($user_id);
  ...
  <div class="flex min-h-screen">
    ...
    <aside class="hidden lg:flex lg:flex-col lg:w-[20%] bg-[color:var(--card-bg)] border-r border-gray-200 dark:border-gray-800 fixed h-full">
      ...
      <nav class="flex-1 p-4 space-y-2">
        ...
        <a href="affichage_inc.php" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 transition-all">
          <span class="material-symbols-outlined">trending_up</span>
          <span class="font-medium">View Incomes</span>
        </a>

        <a href="affichage_exp.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-[color:var(--accent-light)] text-white transition-all hover:opacity-90">
    ...
    <div id="mobile-sidebar" class="hidden fixed inset-0 z-50 lg:hidden">
      ...
      <aside class="absolute left-0 top-0 h-full w-64 bg-[color:var(--card-bg)] border-r border-gray-200 dark:border-gray-800 flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-2">
          ...
          <a href="affichage_exp.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-[color:var(--accent-light)] text-white transition-all">
            <span class="material-symbols-outlined">trending_down</span>
            <span class="font-medium">View Expenses</span>
          </a>
        </nav>
    ...
    <div class="flex-1 lg:ml-[20%]">
      ...
      <main class="p-4 sm:p-6 lg:p-8">
                  ...
                  <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td class="px-4 py-4">
                      <div class="flex justify-end gap-2">
                        ...
                        <!-- <a href="delete_exp.php?id=" onclick="return confirm('Are you sure you want to delete this expense record?');">
                        ...
                        <button
                          class="flex items-center gap-1 px-3 py-2 rounded-lg text-white bg-red-600 hover:bg-red-700 transition shadow-sm"
        ...
        <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center">
          ...
          <div class="relative bg-white dark:bg-[color:var(--card-bg)] rounded-2xl shadow-lg w-full max-w-md p-6">

            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
              Confirm deletion
            </h3>

            <p class="mt-2 text-sm text-gray-500 dark:text-[color:var(--muted)]">
              Are you sure you want to delete this expense?
              This action cannot be undone.
            </p>

            <form method="POST" action="" class="mt-6 flex justify-end gap-3">
              <input type="hidden" name="id" id="delete-expense-id">

              <button type="button"
                data-action="close-delete-modal"
                class="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5">
                Cancel
              </button>

              <button type="submit"
                class="px-4 py-2 rounded-xl text-white bg-red-600 hover:bg-red-700 transition">
                Yes, delete
              </button>
            </form>
        ...
        <script>
          document.addEventListener("DOMContentLoaded", function() {
            ...
            const deleteInput = document.getElementById('delete-expense-id');

            document.addEventListener('click', function(e) {
              const openBtn = e.target.closest("[data-action='open-delete-modal']");
              const closeBtn = e.target.closest("[data-action='close-delete-modal']");

              if (openBtn) {
                deleteInput.value = openBtn.dataset.id;
                deleteModal.classList.remove('hidden');
                deleteModal.classList.add('flex');
              }

              if (closeBtn || e.target === deleteModal) {
